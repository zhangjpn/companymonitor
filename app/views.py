...
@app.route(r'/commonapi/admin/statistics/companies', methods=['GET'])
def census_companies():
    """统计一段时间内每天活跃企业数"""
    # 参数处理
    default_date = datetime.now().strftime('%Y-%m-%d')
    start = str_to_date(request.args.get('from', default_date))
    end = str_to_date(request.args.get('to', default_date))
...
@app.route(r'/commonapi/admin/statistics/comments', methods=['GET'])
def census_comments():
    """统计一段时间内辖区评论数"""
    # 参数处理
    default_date = datetime.now().strftime('%Y-%m-%d')
...
@app.route(r'/commonapi/admin/statistics/complaints', methods=['GET'])
