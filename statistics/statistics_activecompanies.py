...
def statistics_activecompanies(citycode, startdate='2016-01-01'):
    """
    ...
    for maintenace in maintenaces:
        companyinfo = maintenace.get('company')
        if not companyinfo:
            continue
        if str(int(companyinfo[0].get('cityCode'))) == citycode:
            ...
            for day in days:
                ...
                for maintenace in v:
                    if in_date(day, maintenace.get('created')):
                        count += 1
                        distinct_companies.add(maintenace.get('company')[0].get('_id'))
                if count > 0:
                    daily_data = {
                        'provinceCode': citycode[0:2] + '0000',
                        'cityCode': citycode,
                        ...
                        'activeCompany': list(distinct_companies),
                        'activeCompaniesQty': len(distinct_companies)
                    }
                else:
                    daily_data = {
                        'provinceCode': citycode[0:2] + '0000',
...
if __name__ == '__main__':
    statistics_activecompanies('371100')
